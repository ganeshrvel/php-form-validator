...
class FormValidator {
	...
	private $custom_array;//array
	public $value_array;//array

	public function __construct( $custom_array_param = null ) {
		$this->error = false;
		$this->error_field_id = null;
		$this->error_msg = array ();
		$this->value_array = array ();

		if ( isset( $custom_array_param ) && $custom_array_param !== null && is_array( $custom_array_param ) && ! empty( $custom_array_param ) ) {
			$this->custom_array = $custom_array_param;
		}
	}
	 ...
	 * item for validation: trim, is empty, is array, is object, is number, sanitize, input sanitize
	 *
	 * @return null
	 */
	public function set_rules( $param = array () ) {
		$type = 'post';
		$value = null;
		$array_allowed = false;

		global $wpdb;
		if ( ! isset( $param ) ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'parameter_invalid', 'rule - param not set', true );

			return null;
		}

		if ( ! is_array( $param ) || count( $param ) < 1 ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'parameter_not_array', 'rule - param not array', true );

			return null;
		}

		#placeholder
		if ( ! isset( $param['placeholder'] ) || trim( $param['placeholder'] ) == '' ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'placeholder_missing', 'rule - placeholder is missing', true );

			return null;
		}
		$placeholderVar = trim( $param['placeholder'] );

		#name
		if ( ! isset( $param['name'] ) || trim( $param['name'] ) === '' ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'name_missing', sprintf( "%s : rule - name is missing", $placeholderVar ), true );
		...
		#custom_error
		$custom_error = '';
		if ( isset( $param['custom_error'] ) && trim( $param['custom_error'] ) !== "" ) {
			$custom_error = $param['custom_error'];
		}

		$nameVar = $param['name'];
		if ( isset( $param['type'] ) ) {
			$typeVar = trim( strtolower( $param['type'] ) );
			switch ( $typeVar ) {
				case 'post':
				case '':
				default:
		...
		#allow_array
		if ( isset( $param['allow_array'] ) && $param['allow_array'] == false ) {
			//array check
			if ( is_array( $this->$type( $nameVar, false, true ) ) ) {
				$this->set_error( $nameVar, sprintf( "Invalid input for %s", $placeholderVar ), false, $custom_error );

				return null;
			}
			$value = $this->$type( $nameVar );
		} else if ( isset( $param['allow_array'] ) && $param['allow_array'] == true ) {
			$array_allowed = true;
			$value = $this->$type( $nameVar, false, true );
		} else {
			$value = $this->$type( $nameVar );
		}

		#is_array
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['is_array'] ) && $param['is_array'] == true && ! is_array( $value ) ) {
			$this->set_error( $nameVar, sprintf( "Invalid input for %s", $placeholderVar ), false, $custom_error );
		...
		#trim
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['trim'] ) && $param['trim'] == true ) {
			$value = $this->$type( $nameVar, true, $array_allowed );
		}

		#sanitize_input
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['sanitize_input'] ) && $param['sanitize_input'] == true ) {
			$value = $this->callback_funct_sanitize_input( $value );
		}

		#required
		if ( isset( $param['required'] ) && $param['required'] == true && ! $this->callback_funct_required( $value ) ) {
			$this->set_error( $nameVar, sprintf( "%s is required", $placeholderVar ), false, $custom_error );

			return null;
		}

		#matches
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['matches'] ) ) {
			if ( ! is_array( $param['matches'] ) || count( $param['matches'] ) != 2 ) {
				$this->set_error( $nameVar,
					sprintf( "%s : rule - matches must be an array; array size must be 2", $placeholderVar ),
					true,
					$custom_error );

				return null;
			} elseif ( ! $this->callback_funct_matches( $value, $param['matches'][1] ) ) {
				$this->set_error( $nameVar,
					sprintf( "%s must be same as %s", $placeholderVar, $param['matches'][0] ),
					false,
					$custom_error );
		...
		#differs
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['differs'] ) ) {
			if ( ! is_array( $param['differs'] ) || count( $param['differs'] ) != 2 ) {
				$this->set_error( $nameVar,
					sprintf( "%s : rule - differs must be an array; array size must be 2", $placeholderVar ),
					true,
					$custom_error );

				return null;
			} elseif ( $this->callback_funct_differs( $value, $param['differs'][1] ) ) {
				$this->set_error( $nameVar,
					sprintf( "%s must not be same as %s", $placeholderVar, $param['differs'][0] ),
					false,
					$custom_error );
		...
		#is_unique
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['is_unique'] ) ) {
			if ( ! is_array( $param['is_unique'] ) || count( $param['is_unique'] ) != 2 ) {
				$this->set_error( $nameVar,
					sprintf( "%s : rule - is_unique must be an array; array size must be 2", $placeholderVar ),
					true,
					$custom_error );

				return null;
			}
			$is_unique_results = null;
			$tableName = $param['is_unique'][0];
			$colName = $param['is_unique'][1];

			$return_is_unique = $this->callback_funct_is_unique( $value, $tableName, $colName );
			if ( is_array( $return_is_unique ) && $return_is_unique['is_unique'] == false ) {
				$this->set_error( $nameVar,
					sprintf( "The %s (%s) already exists.", $placeholderVar, $return_is_unique['existing_value'] ),
					false,
					$custom_error );

				return null;
			} elseif ( ! is_array( $return_is_unique ) && $return_is_unique != false ) {
				$this->set_error( $nameVar,
					sprintf( "The %s (%s) already exists.", $placeholderVar, $value ),
					false,
					$custom_error );
		...
		#is_duplicate
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['is_duplicate'] ) ) {
			if ( ! is_array( $param['is_duplicate'] ) || ! isset( $param['is_duplicate']['table_name'] ) || ! isset( $param['is_duplicate']['column_name'] ) || ! isset( $param['is_duplicate']['primary_key'] ) || ! isset( $param['is_duplicate']['primary_key_value'] ) || count( $param['is_duplicate'] ) != 4 ) {
				$this->set_error( $nameVar,
					sprintf( "%s : rule - is_duplicate must be an array; array size must be 4", $placeholderVar ),
					true,
					$custom_error );
			...
			$is_unique_results = null;
			$check_results = null;
			$tableName = $param['is_duplicate']['table_name'];
			$colName = $param['is_duplicate']['column_name'];
			$primaryKey = $param['is_duplicate']['primary_key'];
			$primaryKeyValue = $param['is_duplicate']['primary_key_value'];
			$is_unique_results = $wpdb->get_var( "SELECT " . $colName . " FROM " . $tableName . " WHERE " . $colName . " = '" . esc_sql( $value ) . "'" );

			if ( $is_unique_results == $value ) {
				$check_results = $wpdb->get_var( "SELECT " . $colName . " FROM " . $tableName . " WHERE " . $colName . " = '" . esc_sql( $value ) . "' AND " . $primaryKey . " = '" . esc_sql( $primaryKeyValue ) . "'" );
				if ( trim( $check_results ) != trim( $value ) ) {
					$this->set_error( $nameVar,
						sprintf( "The %s (%s) already exists.", $placeholderVar, $is_unique_results ),
						false,
						$custom_error );
		...
		#min_length
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['min_length'] ) && $this->callback_funct_min_length( $value,
				$param['min_length'] ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain at least %s character(s)", $placeholderVar, (int) $param['min_length'] ),
				false,
				$custom_error );
		...
		#max_length
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['max_length'] ) && $this->callback_funct_max_length( $value,
				$param['max_length'] ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must not exceed %s character(s)", $placeholderVar, (int) $param['max_length'] ),
				false,
				$custom_error );
		...
		#exact_length
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['exact_length'] ) && $this->callback_funct_exact_length( $value,
				$param['exact_length'] ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain exactly %s character(s)", $placeholderVar, (int) $param['exact_length'] ),
				false,
				$custom_error );
		...
		#greater_than
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['greater_than'] ) && $value <= $param['greater_than'] ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be greater than %s", $placeholderVar, (int) $param['greater_than'] ),
				false,
				$custom_error );
		...
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['greater_than_equal_to'] ) && $value < $param['greater_than_equal_to'] ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be greater than or equal to %s",
					$placeholderVar,
					(int) $param['greater_than_equal_to'] ),
				false,
				$custom_error );
		...
		#less_than
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['less_than'] ) && $value >= $param['less_than'] ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be less than %s", $placeholderVar, (int) $param['less_than'] ),
				false,
				$custom_error );
		...
		#less_than_equal_to
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['less_than_equal_to'] ) && $value > $param['less_than_equal_to'] ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be less than or equal to %s", $placeholderVar, (int) $param['less_than_equal_to'] ),
				false,
				$custom_error );
		...
		#in_list
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['in_list'] ) ) {
			if ( ! is_array( $param['in_list'] ) || empty( $param['in_list'] ) ) {
				$this->set_error( $nameVar,
					sprintf( "%s : rule - in_list must be an array; array size must be at least 1", $placeholderVar ),
					true,
					$custom_error );

				return null;
			}
			if ( ! $this->callback_funct_in_list( $value, $param['in_list'] ) ) {
				$this->set_error( $nameVar,
					sprintf( "%s must be any of the following: %s...",
						$placeholderVar,
						implode( ", ", array_slice( $param['in_list'], 0, 10 ) ) ),
					false,
					$custom_error );
		...
		#min_item_limit
		if ( is_array( $value ) && isset( $param['min_item_limit'] ) && $param['min_item_limit'] == true && $this->callback_funct_min_item_limit( $value,
				$param['min_item_limit'] ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain at least %s item(s)", $placeholderVar, (int) $param['min_item_limit'] ),
				false,
				$custom_error );
		...
		#max_item_limit
		if ( is_array( $value ) && isset( $param['max_item_limit'] ) && $param['max_item_limit'] == true && $this->callback_funct_max_item_limit( $value,
				$param['max_item_limit'] ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must not exceed %s item(s)", $placeholderVar, (int) $param['max_item_limit'] ),
				false,
				$custom_error );
		...
		#exact_item_limit
		if ( is_array( $value ) && isset( $param['exact_item_limit'] ) && $param['exact_item_limit'] == true && ! $this->callback_funct_exact_item_limit( $value,
				$param['exact_item_limit'] ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain exactly %s item(s)", $placeholderVar, (int) $param['exact_item_limit'] ),
				false,
				$custom_error );
		...
		#unique_items
		if ( is_array( $value ) && isset( $param['unique_items'] ) && $param['unique_items'] == true && count( $value ) !== count( array_unique( $this->callback_funct_unique_items( $value ) ) ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must not contain duplicate item(s)", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha'] ) && $param['alpha'] == true && preg_match( "/^[a-zA-Z]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha_space
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_space'] ) && $param['alpha_space'] == true && preg_match( "/^[a-zA-Z ]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets and spaces", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha_numeric
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_numeric'] ) && $param['alpha_numeric'] == true && preg_match( "/^[a-zA-Z0-9]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets and numbers", $placeholderVar ),
				false,
				$custom_error );
		...
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_numeric_spaces'] ) && $param['alpha_numeric_spaces'] == true && preg_match( "/^[a-zA-Z0-9 ]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets, numbers and spaces", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha_dash
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_dash'] ) && $param['alpha_dash'] == true && preg_match( "/^[a-zA-Z-_]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets, hyphens and underscores", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha_numeric_dash
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_numeric_dash'] ) && $param['alpha_numeric_dash'] == true && preg_match( "/^[a-zA-Z0-9-_]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets, numbers, hyphens and underscores", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha_dot
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_dot'] ) && $param['alpha_dot'] == true && preg_match( "/^[a-zA-Z.]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets and periods(.)", $placeholderVar ),
				false,
				$custom_error );
		...
		#alpha_numeric_dot
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['alpha_numeric_dot'] ) && $param['alpha_numeric_dot'] == true && preg_match( "/^[a-zA-Z0-9.]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s must contain only alphabets, numbers and periods(.)", $placeholderVar ),
				false,
				$custom_error );
		...
		#integer
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['integer'] ) && $param['integer'] == true && ! filter_var( $value,
				FILTER_VALIDATE_INT ) ) {
			$this->set_error( $nameVar, sprintf( "%s must be an integer", $placeholderVar ), false, $custom_error );

			return null;
		}

		#numeric
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['numeric'] ) && $param['numeric'] == true && ! $this->callback_funct_numeric( $value ) ) {
			$this->set_error( $nameVar, sprintf( "%s must be numeric", $placeholderVar ), false, $custom_error );

			return null;
		}

		#decimal
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['decimal'] ) && $param['decimal'] == true ) {
			if ( ! is_numeric( $value ) || strpos( $value, "." ) === false ) {
				$this->set_error( $nameVar, sprintf( "%s must be a decimal", $placeholderVar ), false, $custom_error );
		...
		#allow_special_chars
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['allow_special_chars'] ) && $param['allow_special_chars'] == false && preg_match( "/^[a-zA-Z0-9 ]+$/",
				$value ) == 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s should not contain any special characters or symbols", $placeholderVar ),
				false,
				$custom_error );
		...
		#allow_space
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['allow_space'] ) && $param['allow_space'] == false && preg_match( '/\s/',
				$value ) != 0 ) {
			$this->set_error( $nameVar,
				sprintf( "%s should not contain any whitespace", $placeholderVar ),
				false,
				$custom_error );
		...
		#allow_alpha
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['allow_alpha'] ) && $param['allow_alpha'] == false && preg_match( "/[a-zA-Z]/i",
				$value ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s should not contain any alphabet", $placeholderVar ),
				false,
				$custom_error );
		...
		#allow_number
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['allow_number'] ) && $param['allow_number'] == false && preg_match( "/[0-9]/i",
				$value ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s should not contain any number", $placeholderVar ),
				false,
				$custom_error );
		...
		#valid_url
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['valid_url'] ) && $param['valid_url'] == true ) {
			//Adding http:// if no protocols is already defined
			$value = $this->callback_preSanitizeURL( $value );
			if ( ! $this->callback_funct_valid_url( $value ) ) {
				$this->set_error( $nameVar,
					sprintf( "%s must be a valid URL", $placeholderVar ),
					false,
					$custom_error );
		...
		#valid_email
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['valid_email'] ) && $param['valid_email'] == true && ! $this->callback_funct_valid_email( $value ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be a valid e-mail address", $placeholderVar ),
				false,
				$custom_error );
		...
		#valid_phone
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['valid_phone'] ) && $param['valid_phone'] == true && ! $this->callback_funct_valid_phone( $value ) ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be a valid phone number", $placeholderVar ),
				false,
				$custom_error );
		...
		#valid_date
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['valid_date'] ) ) {
			if ( $param['valid_date'] == true && (bool) strtotime( $value ) == false ) {
				$this->set_error( $nameVar,
					sprintf( "%s must be a valid date", $placeholderVar ),
					false,
					$custom_error );
			...
			if ( isset( $param['date_range'] ) ) {
				if ( ! is_array( $param['date_range'] ) || count( $param['date_range'] ) != 2 || ! isset( $param['date_range']['min'] ) || ! isset( $param['date_range']['max'] ) ) {
					$this->set_error( $nameVar,
						sprintf( "%s : rule - differs must be an array; array size must be 2", $placeholderVar ),
						true,
						$custom_error );
				...
				if ( ! ( $value >= $param['date_range']['min'] ) || ! ( $value <= $param['date_range']['max'] ) ) {
					$this->set_error( $nameVar,
						sprintf( "%s must be within %s to %s date range",
							$placeholderVar,
							$param['date_range']['min'],
							$param['date_range']['max'] ),
						false,
						$custom_error );
		...
		#valid_ip
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['valid_ip'] ) && $param['valid_ip'] == true && filter_var( $value,
				FILTER_VALIDATE_IP ) === false ) {
			$this->set_error( $nameVar,
				sprintf( "%s must be a valid IP address", $placeholderVar ),
				false,
				$custom_error );
		...
		#file_extension
		if ( $type == 'files' && ( is_array( $value ) || ! empty( $value ) ) && isset( $param['file_extension'] ) ) {
			if ( ! is_array( $param['file_extension'] ) || count( $param['file_extension'] ) < 1 ) {
				$this->set_error( $nameVar,
					sprintf( "%s : files rule - file_extension must be an array; array size must be at least 1",
						$placeholderVar ),
					true,
					$custom_error );
			...
			$allowed_extension_array = $param['file_extension'];
			$file_ext_error_flag = false;
			$file_ext_counter = 0;
			if ( ! isset( $value['type'] ) ) {
				$file_ext_error_flag = true;//error
			}
			$uploaded_file_mime_type = $value['type'];
			$mime_types_list = $this->mime_types();
			if ( ! $file_ext_error_flag ) {
				foreach ( $allowed_extension_array as $ext_item ) {
					$ext_item = strtolower( $ext_item );
					if ( ! isset( $mime_types_list[ $ext_item ] ) ) {
						$file_ext_error_flag = true;//error
						break;
					} else if ( in_array( $uploaded_file_mime_type, $mime_types_list[ $ext_item ] ) ) {
						$file_ext_counter ++;//found
						break;
					}
				}
			}

			if ( $file_ext_error_flag || $file_ext_counter == 0 ) {
				$this->set_error( $nameVar,
					sprintf( "%s file format is not allowed. Allowed extensions: %s.",
						$placeholderVar,
						implode( ", ", $allowed_extension_array ) ),
					false,
					$custom_error );
		...
		#file_max_size
		if ( $type == 'files' && ( is_array( $value ) || ! empty( $value ) ) && isset( $param['file_max_size'] ) ) {
			if ( ( ! isset( $value["size"] ) || $value["size"] == 0 ) ) {
				$this->set_error( $nameVar,
					sprintf( "%s file too small. Upload a valid file", $placeholderVar ),
					false,
					$custom_error );

				return null;
			} else if ( $value['size'] > $param['file_max_size'] ) {
				$this->set_error( $nameVar,
					sprintf( "%s file too large. File must be less than %s",
						$placeholderVar,
						$this->format_size_units( (int) $param['file_max_size'] ) ),
					false,
					$custom_error );
		...
		#sql_escape
		if ( ! ( $value !== 0 && empty( $value ) ) && isset( $param['sql_escape'] ) && $param['sql_escape'] == true ) {
			$value = $this->$type( $nameVar, false, $array_allowed, true );
		} elseif ( isset( $param['sql_escape'] ) && $param['sql_escape'] == false ) {
			$value = $this->$type( $nameVar );
		}
		$this->value_array[ $nameVar ] = array ( 'placeholder' => $placeholderVar, 'value' => $value );

		return $value;
	}

	/**Description: Group set rules
	 *
	 *
	 * @param $param array
	 *
	 * @return null|boolean
	 */
	public function group_set_rules( $param = array () ) {
		if ( ! isset( $param ) ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'group_set_parameter_invalid', 'group set rule - param not set', true );

			return null;
		}

		if ( ! is_array( $param ) || count( $param ) < 1 ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'group_set_parameter_not_array', 'group set rule - param not array', true );

			return null;
		}

		#group_name
		if ( ! isset( $param['group_name'] ) || trim( $param['group_name'] ) === '' ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'group_set_name_not_set', 'group set rule - name not set', true );

			return null;
		}
		$groupNameVar = $param['group_name'];

		#group_placeholder
		if ( ! isset( $param['group_placeholder'] ) || trim( $param['group_placeholder'] ) === '' ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'group_set_placeholder_not_set', 'group set rule - placeholder not set', true );

			return null;
		}
		$groupPlaceholderVar = $param['group_placeholder'];

		#items
		if ( ! isset( $param['items'] ) || ! is_array( $param['items'] ) || count( $param['items'] ) < 1 ) { #Mandatory parameter; for developer debugging
			$this->set_error( 'group_set_items_not_set', 'group set rule - items not set', true );

			return null;
		}
		$itemsVar = $param['items'];
		...
		#min_item_selected
		if ( isset( $param['min_item_selected'] ) && $param['min_item_selected'] == true ) {
			$count_flag = 0;
			$error_flag = false;
			$placeholdersVar = '';
			foreach ( $itemsVar as $item ) {
				if ( ! isset( $this->value_array[ $item ] ) ) {
					$this->set_error( 'group_set_invalid_field_name', 'group set rule - invalid field name', true );
					$error_flag = true;

					break;
				} elseif ( $this->value_array[ $item ]['value'] !== '' && $this->value_array[ $item ]['value'] !== null && ! empty( $this->value_array[ $item ]['value'] ) ) {
					$count_flag ++;
				}
				$placeholdersVar .= $this->value_array[ $item ]['placeholder'] . ', ';
			}
			$placeholdersVar = implode( ", ", array_filter( explode( ", ", $placeholdersVar ) ) );
			if ( $error_flag ) {
				$this->set_error( $groupNameVar,
					sprintf( "Select at least %s %s field(s)",
						(int) $param['min_item_selected'],
						$groupPlaceholderVar ),
					false,
					$custom_error );
			} elseif ( $count_flag < $param['min_item_selected'] ) {
				$this->set_error( $groupNameVar,
					sprintf( "Select at least %s %s field(s) (%s)",
						(int) $param['min_item_selected'],
						$groupPlaceholderVar,
						$placeholdersVar ),
					false,
	 ...
	 * @param bool $trim : trim the output (only for non array inputs)
	 ...
	 * @return null|string
	 */
	public function post( $param, $trim = false, $isArray = false, $sql_escape = false ) {
		return $this->preSanitize( $param, $_POST, $trim, $isArray, $sql_escape );
	}
	 ...
	 *
	 * @param $param : form input field name
	 * @param $trim : trim the output (only for non array inputs)
	 * @param bool $isArray : true if an array value is expected from the form input
	 * @param bool $sql_escape : escape the input value against sql injections
	 *
	 * @return null|string
	 */
	public function get( $param, $trim = false, $isArray = false, $sql_escape = false ) {
		return $this->preSanitize( $param, $_GET, $trim, $isArray, $sql_escape );
	}
	 ...
	 * @return null|string
	 */
	public function custom( $param, $trim = false, $isArray = false, $sql_escape = false ) {
		return $this->preSanitize( $param, $this->custom_array, $trim, $isArray, $sql_escape );
	}
	 ...
	 * @return null|string
	 */
	public function request( $param, $trim = false, $isArray = false, $sql_escape = false ) {
		return $this->preSanitize( $param, $_REQUEST, $trim, $isArray, $sql_escape );
	}
	 ...
	 * @return null|string
	 */
	public function files( $param, $trim = false, $isArray = false, $sql_escape = false ) {
		return $this->preSanitize( $param, $_FILES, $trim, $isArray, $sql_escape );
	}

	/**
	 * Show form errors - Form validation;
	 *
	 * @param integer $limit ;number of errors to be displayed
	 * @param bool $disallow_styles ;remove the divs, classes and the styles applied to the errors
	 * @param $errors_seperator ;a 'special character' to seperate multiple errors [Only valid while $strip_styles = true]
	 * @param bool $debugging ;enable only for debugging
	 *
	 * @return null|string
	 */
	public function show_error( $limit = 1, $disallow_styles = true, $errors_seperator = "|", $debugging = false ) {
		return $this->show_all_errors( $limit, $disallow_styles, $debugging, $errors_seperator );
	}

	/**
	 * Show ALL form errors - Form validation;
	 *
	 * @param null $limit ;number of errors to be displayed
	 * @param bool $strip_styles ;remove the divs, classes and the styles applied to the errors
	 * @param $errors_seperator ;a 'special character' to seperate multiple errors [Only valid while $strip_styles = true]
	 * @param bool $debugging ;enable only for debugging
	 *
	 * @return null|string
	...
	public function show_all_errors(
		...
		$errorString = "";
		if ( empty( $this->error_msg ) && $this->error == false ) {
			return null;
		}
		if ( ! $strip_styles ) {
			$li_open = '<li>';
			...
			$li_open = "";
			$li_close = $errors_seperator;
		}
		$temp_count = 0;
		foreach ( $this->error_msg as $key => $error ) {
			if ( $limit && is_numeric( $limit ) && $limit <= $temp_count ) {
				break;
			}
			if ( $debugging == false && $key == '[debugging]' ) {
				continue;
			} elseif ( $debugging == true && $key == '[debugging]' ) {
				foreach ( $error as $debugError ) {
					if ( $limit && is_numeric( $limit ) && $limit <= $temp_count ) {
						break;
					}
					$errorString .= $li_open . $debugError . $li_close;
					$temp_count ++;
				}
				continue;
			}
			$temp_count ++;
			$errorString .= $li_open . $error . $li_close;
		}
		if ( $strip_styles ) {
			return implode( $errors_seperator, array_filter( explode( $errors_seperator, $errorString ) ) );
		}

		return '<div class="alert alert-block fade in custom-style-message-box custom-style-message-error"> 
				<strong class="custom-style-message-desc">Errors:</strong>
				<hr class="custom-style-message-hr custom-style-message-hr-error" />
				<div class="custom-style-message-text">' . $errorString . '</div>
			  </div>';
	}


	/**Form validation error for a single field name
	 *
	 * @param $fieldName
	 *
	 * @return string|null
	 */
	public function show_error_item( $fieldName ) {
		if ( empty( $this->error_msg ) || empty( $fieldName ) ) {
			return null;
		}
		if ( ! isset( $this->error_msg[ $fieldName ] ) ) {
			return null;
		}

		return $this->error_msg[ $fieldName ];
	}

	/**
	 * Run the validation test
	 * @return bool: true if it passes validation else false
	 */
	public function run() {
		if ( count( $this->error_msg ) == 0 && $this->error == false ) {
			return true;
		}
	...
	/**
	 * Pre Sanitize the input requests; private
	 *
	 * @param $param
	 * @param $requestType ($_POST, $_GET, $_RESQUEST, $_FILES)
	 * @param $trim
	 * @param $isArray
	 * @param $sql_escape
	 *
	 * @return null|string
	 */
	private function preSanitize( $param, $requestType, $trim, $isArray, $sql_escape ) {
		if ( ! isset( $param ) || ! isset( $requestType[ $param ] ) ) {
			return null;
		}
		if ( ! $isArray && is_array( $requestType[ $param ] ) ) {
			return null;
		} elseif ( $isArray && ! is_array( $requestType[ $param ] ) ) {
			return null;
		}
		if ( $sql_escape && ! $isArray ) {
			$requestType[ $param ] = esc_sql( $requestType[ $param ] );
		} elseif ( $sql_escape && $isArray ) {
			$requestType[ $param ] = $this->callback_funct_sql_escape( $requestType[ $param ] );
		}
		if ( $trim && ! $isArray ) {
			return trim( $requestType[ $param ] );
		} elseif ( $trim && $isArray ) {
			return $this->callback_funct_trim( $requestType[ $param ] );
		}
	 ...
	 * @param null $custom_error
	 */
	private function set_error( $fieldName, $errorMessage, $debugging = false, $custom_error = null ) {
		$this->error = true;
		if ( $debugging ) {
			if ( ! $this->error_field_id ) {
				$this->error_field_id = $fieldName;
			}
			$this->error_msg['[debugging]'] = array ( $fieldName => $errorMessage );
		} elseif ( $custom_error ) {
			$this->error_msg[ $fieldName ] = $custom_error;
		} else {
			$this->error_msg[ $fieldName ] = $errorMessage;
		}
		if ( ! $this->error_field_id ) {
			$this->error_field_id = $fieldName;
		}
	...
	#required
	private function callback_funct_required( $value ) {
		if ( is_array( $value ) ) {
			if ( in_array( false, array_map( array ( $this, 'callback_funct_required' ), $value ) ) ) {
	...
	#trim
	private function callback_funct_trim( $value ) {
		if ( is_array( $value ) ) {
			return array_map( array ( $this, 'callback_funct_trim' ), $value );
		}
	...
	#differs
	private function callback_funct_differs( $value, $match_value ) {

		if ( is_array( $value ) ) {
			if ( ! is_array( $match_value ) ) {
				$match_value_array = array_fill( 0, count( $value ), $match_value );
			} else {
				$match_value_array = $match_value;
			}

			if ( in_array( true,
				array_map( array (
					$this,
					'callback_funct_differs',
				),
					$value,
					$match_value_array ) ) ) {
				return true;
	...
	#is_unique
	private function callback_funct_is_unique( $value, $tableName, $colName ) {
		global $wpdb;
		if ( is_array( $value ) ) {

			if ( ! is_array( $tableName ) ) {
				$tableName_array = array_fill( 0, count( $value ), $tableName );
			} else {
				$tableName_array = $tableName;
			}

			if ( ! is_array( $colName ) ) {
				$colName_array = array_fill( 0, count( $value ), $colName );
			} else {
				$colName_array = $colName;
			}
			$mapped_values = array_map( array (
				$this,
				'callback_funct_is_unique',
			),
				$value,
				$tableName_array,
				$colName_array );

			$key = array_search( true, $mapped_values );

			if ( $key !== false ) {
				return array ( 'is_unique' => false, 'existing_value' => $value[ $key ] );
			}

			return array ( 'is_unique' => true, 'existing_value' => null );
		}

		$is_unique_results = null;
		$is_unique_results = $wpdb->get_var( "SELECT " . $colName . " FROM " . $tableName . " WHERE " . $colName . " = '" . esc_sql( $value ) . "'" );

		return $is_unique_results == $value;
	}

	#matches
	private function callback_funct_matches( $value, $match_value ) {
	...
	#min_length
	private function callback_funct_min_length( $value, $match_value ) {
		...
		return strlen( $value ) < (int) $match_value;
	}

	#max_length
	private function callback_funct_max_length( $value, $match_value ) {
	...
	#exact_length
	private function callback_funct_exact_length( $value, $match_value ) {
	...
	#in_list
	private function callback_funct_in_list( $value, $match_value ) {
		if ( is_array( $value ) ) {
			$match_value_array = array_fill( 0, count( $value ), $match_value );
			if ( in_array( false,
				array_map( array (
					$this,
					'callback_funct_in_list',
				),
					$value,
					$match_value_array ) ) ) {
		...
		if ( array_keys( $match_value ) !== range( 0, count( $match_value ) - 1 ) ) {

			return array_key_exists( $value, $match_value );
		}

		return in_array( $value, $match_value );
	}

	#min_item_limit
	private function callback_funct_min_item_limit( $value, $match_value ) {
		if ( ! is_array( $value ) ) {
			return null;
		}

		return count( $value ) < (int) $match_value;
	}

	#max_item_limit
	private function callback_funct_max_item_limit( $value, $match_value ) {
		if ( ! is_array( $value ) ) {
			return null;
		}
	...
	#exact_item_limit
	private function callback_funct_exact_item_limit( $value, $match_value ) {
		if ( ! is_array( $value ) ) {
			return null;
		}
	...
	#unique_items
	private function callback_funct_unique_items( $param ) {
		return array_intersect_key( $param, array_unique( array_map( "strtolower", $param ) ) );
	}

	#numeric
	private function callback_funct_numeric( $value ) {

		if ( is_array( $value ) ) {
			if ( in_array( false,
				array_map( array (
					$this,
					'callback_funct_numeric',
				),
					$value ) ) ) {
	...
	#SanitizeURL
	private function callback_preSanitizeURL( $value ) {
		if ( is_array( $value ) ) {
			return array_map( array ( $this, 'callback_preSanitizeURL' ), $value );
		}

		return parse_url( $value, PHP_URL_SCHEME ) === null ? 'http://' . $value : $value;
	}

	#valid_url
	private function callback_funct_valid_url( $value ) {
	...
	#valid_email
	private function callback_funct_valid_email( $value ) {
	...
	#valid_phone
	private function callback_funct_valid_phone( $value ) {
		...
		return preg_match( '/^(?:(?:\+|0{0,2})91(\s*[\ -]\s*)?|[0]?)?[789]\d{9}|(\d[ -]?){10}\d$/', $value );
	}

	#sql_escape
	private function callback_funct_sql_escape( $value ) {
		if ( is_array( $value ) ) {
			return array_map( array ( $this, 'callback_funct_sql_escape' ), $value );
		}
	...
	#sanitize_input
	private function callback_funct_sanitize_input( $value ) {
		if ( is_array( $value ) ) {
			return array_map( array ( $this, 'callback_funct_sanitize_input' ), $value );
		}
